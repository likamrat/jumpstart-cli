...
# - Error message format validation for required arguments
...
test_error_message "arcbox delete" "js arcbox delete (missing name)" "the following arguments are required:.*name"
...
test_error_message "completion" "js completion (missing shell)" "the following arguments are required:.*_subcommand"
test_error_message "arcbox preflight rp register" "js arcbox preflight rp register (missing name)" "the following arguments are required:.*name"
...
if [ $TOTAL_ISSUES -eq 0 ]; then
    ...
    echo ""
    echo "🔧 Fix needed before completion"
    exit 1
fi
