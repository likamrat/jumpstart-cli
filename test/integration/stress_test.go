	...
	for _, test := range networkSensitiveCommands {
		t.Run(test.name, func(t *testing.T) {
			...
			case err := <-done:
				...
				// Either produce an error (network failure detected) or output (command succeeded despite network issues)
				// For subscription list, the command may succeed by calling Azure CLI which handles network issues itself
				if test.name == "subscription_list" {
					// subscription list may succeed even under network stress as it calls Azure CLI
					// which has its own network handling - this is acceptable behavior
					t.Logf("Subscription list executed - Azure CLI may have handled network issues internally")
				} else {
